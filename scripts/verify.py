...
# Reports are UTF-8, including on Windows where the legacy console/code page can
# otherwise crash the verifier before it reaches the actual checklist.
...
def is_committed(path: pathlib.Path) -> bool | None:
    ...
    if TRACKED is None:
        return None
    try:
        # Git reports POSIX separators even on Windows; normalize the local path
        # before comparing so committed artifacts are recognized correctly.
